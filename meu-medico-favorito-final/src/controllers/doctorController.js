...
const updateFavorite = async (req, res) => {
  ...
  try {
    ...
    if (rowsUpdated && rowsUpdated > 0) {
      ...
      res.status(404).send({
        message: `Medico com id ${doctorId} não encontrado para atualizar informação de favorito`,
      });
    }
  } catch (error) {
    res.status(500).send({ message: error.message });
  ...
  try {
    ...
    if (rowsDeleted) {
      res
        .status(200)
        .send({
          message: `${rowsDeleted[0]} medico(s) deletado(s) com sucesso`,
        });
    } else {
      res
        .status(404)
        .send({
          message: `Medico com id ${doctorId} não encontrado para deletar`,
        });
    }
  } catch (error) {
    res.status(500).send({ message: error.message });
